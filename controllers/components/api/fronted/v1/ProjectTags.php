...
class ProjectTags extends Tag_Controller
{
	private $_ci;
	...
	public function __construct()
	{
		...
		$this->load->model('extensions/FHC-Core-PEP/PEP_Projekt_Notiz_model', 'PEPProjektNotizModel');
	...
	public function addTag($withZuordnung = false, $updatable_tags = null)
	{
		...
		foreach ($postData->values as $value)
		{
			...
			$insertZuordnung = $this->PEPProjektNotizModel->insert(array(
				'notiz_id' => $insertResult,
				'pep_projects_employees_id' => $value
			));

			if (isError($insertZuordnung))
				$this->terminateWithError('Error occurred', self::ERROR_TYPE_GENERAL);
			$return[] = ['pep_projects_employees_id' => $value, 'id' => $insertResult];
		}
	...
	public function deleteTag($withZuordnung = false, $updatable_tags = null)
	{
		...
		$deleteZuordnung = $this->PEPProjektNotizModel->delete(array(
